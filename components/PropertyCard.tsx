...
const percentageTwoDecimalsFormatter = new Intl.NumberFormat("en-US", { style: 'percent', minimumFractionDigits: 0, maximumFractionDigits: 2 });
                ...
                <a href={"/properties/" + id} className="flex flex-1 flex-col space-y-4 px-4 pt-4 justify-end">
                    ...
                    <div className="flex flex-col space-y-2">
                        <div className="flex flex-row justify-between items-end">
                            ...
                            <p className="text-sm font-medium text-gray-600">{percentageZeroDecimalsFormatter.format((totalBalance) ? totalBalance / 10000000 : 0)} funded</p>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                            <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: (totalBalance) ? percentageSixDecimalsFormatter.format(totalBalance / 10000000) : 0 }}></div>
                        </div>
                    </div>
                    <section className="rounded-lg bg-gray-50 p-3">
                        <dl className="space-y-2">
                            <div className="flex items-center justify-between hidden">
                                <dt className="flex text-sm text-gray-600">Market Price</dt>
                                <dd className="text-sm font-medium text-gray-900">{currencyformatter.format(price)}</dd>
                            </div>
                            <div className="flex items-center justify-between hidden">
                            ...
                            <div className="flex items-center justify-between">
                                <dt className="flex text-sm text-gray-600">My Value</dt>
                                <dd className="text-sm font-medium text-gray-900">{currencyformatter.format((myStake) ? price * myStake / 10000000 : 0)}</dd>
                            </div>
                            <div className="flex items-center justify-between">
                                <dt className="flex text-sm text-gray-600">My Shares</dt>
                                <dd className="text-sm font-medium text-gray-900">{(myStake) ? numberFormatter.format(Number(myStake)) : 0}</dd>
                            </div>
                            <div className="flex items-center justify-between">
                                <dt className="flex text-sm text-gray-600">Minted Shares</dt>
